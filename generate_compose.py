...
DEFAULT_PORT = 9009
DEFAULT_GREEN_PORT = 9021
DEFAULT_PURPLE_PORT = 9022
...
services:
  green-agent:
    ...
    container_name: green-agent{suffix}
    command: ["--host", "0.0.0.0", "--port", "{green_port}", "--card-url", "http://green-agent{suffix}:{green_port}"]
...
PARTICIPANT_TEMPLATE = """  {name}:
    ...
    command: ["--host", "0.0.0.0", "--port", "{port}", "--card-url", "http://{name}{suffix}:{port}"]
    environment:{env}
    healthcheck:
...
endpoint = "http://green-agent{suffix}:{green_port}"
...
def generate_docker_compose(scenario: dict[str, Any]) -> str:
    ...
    participant_services = "\n".join([
        PARTICIPANT_TEMPLATE.format(
            name=p["name"],
            image=p["image"],
            port=p["port"] if "port" in p else DEFAULT_PORT,
